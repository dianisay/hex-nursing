...
(18.54\,m; \num{34299}\,MXN), but adjusted wall-sharing accounting reverses this
advantage: the specific hexagonal floor plan requires 12.0\% \emph{more} wall
material than the square plan.
Graph-based nurse route optimisation (Dijkstra + exact Miller--Tucker--Zemlin TSP)
shows that the hexagonal layout's 6-connectivity reduces mean travel distance from
the nursing station by \textbf{20.1\%} (\SI{8.68}{\metre} vs \SI{6.94}{\metre});
the optimal full-round tour is nearly identical (\SI{60.1}{\metre} vs \SI{61.0}{\metre},
$<$2\% difference).
Expert survey (mockup, $n=5$) rated hexagonal circulation higher (4.2 vs 3.4/5)
but implementation willingness lower (3.2 vs 4.2/5).

\noindent\textbf{Conclusions.}
The theoretical 6.94\% material savings reverses at the floor-plan level after
wall-sharing accounting.
The primary quantitative case for hexagonal nursing stations is a 20.1\% reduction in
mean nurse travel distance, confirmed by exact graph-model TSP optimisation, without
a meaningful impact on the optimal full-round tour ($<$2\% difference).
Future work should validate travel-distance findings with discrete-event simulation
in a real Mexican hospital unit and verify NOM-016-SSA3-2012 compliance.
...
\subsection{Graph model and nurse route optimisation}
\label{sec:methods_tsp}

We model each layout as a weighted undirected graph $G=(V,E,w)$ where nodes $V$ are
room centres, edges $E$ connect physically adjacent rooms (rooms sharing a wall or
open corridor), and edge weight $w(e)$ is the Euclidean centre-to-centre distance
in metres.

For the \textbf{square layout}, we construct a standard 4-connected grid graph
on a $4 \times 5$ arrangement.
Each adjacent pair is connected at distance $l_s = \SI{3.339}{\metre}$.

For the \textbf{hexagonal layout}, we construct a 6-connected offset-row hex graph
on 20 modules.
All six neighbours of any hex are equidistant at $l_h\sqrt{3} = \SI{3.588}{\metre}$
(a direct consequence of the apothem identity $a_h = l_h\sqrt{3}/2$).

\paragraph{Shortest-path metric.}
All-pairs shortest-path distances are computed via Dijkstra's algorithm
(NetworkX\footnote{\url{https://networkx.org}}, v3.6).
The resulting $16 \times 16$ distance matrix (1 nursing-station depot + 15 patient rooms)
forms a complete metric graph on which the TSP is solved.

\paragraph{Travelling Salesman Problem.}
We formulate the nurse's daily visit round as a symmetric TSP:
find the minimum-cost Hamiltonian cycle starting and ending at the
nursing-station depot and visiting every patient room exactly once.
The exact solution is obtained via the Miller--Tucker--Zemlin (MTZ)
integer linear programme~\cite{miller1960integer}:
\begin{align}
  \min_{x_{ij}} &\quad \sum_{i \neq j} d_{ij} x_{ij} \label{eq:tsp_obj}\\
  \text{s.t.} &\quad \sum_{i \neq j} x_{ij} = 1 \quad \forall j \label{eq:tsp_in}\\
              &\quad \sum_{j \neq i} x_{ij} = 1 \quad \forall i \label{eq:tsp_out}\\
              &\quad u_i - u_j + n \, x_{ij} \leq n-1 \quad \forall i,j \geq 1,\; i \neq j \label{eq:mtz}\\
              &\quad x_{ij} \in \{0,1\},\; u_i \geq 0 \label{eq:domain}
\end{align}
where $d_{ij}$ is the Dijkstra shortest-path distance between nodes $i$ and $j$,
$x_{ij}=1$ if arc $i \to j$ is in the optimal tour, and $u_i$ is a continuous
position variable enforcing subtour elimination~(Eq.~\ref{eq:mtz}).
The model is implemented in PuLP~3.3 and solved with the bundled CBC solver;
\textbf{Gurobi is used automatically if available} on the host machine
(detected via \texttt{shutil.which('gurobi\_cl')}).
With 16 nodes the problem has 240 binary variables and is solved to proven optimality
in under one second by either solver.
...
\subsection{Travel distance and optimal nurse tour}
\label{sec:results_tsp}

Table~\ref{tab:tsp} summarises the graph-based travel distance metrics for both layouts.

\begin{table}[htbp]
\centering
\caption{Graph-model travel distance comparison (Dijkstra shortest paths + exact MTZ TSP).
$n = 15$ patient rooms, 1 nursing-station depot.}
\label{tab:tsp}
\begin{tabular}{@{}lrrr@{}}
\toprule
Metric & Square & Hexagonal & Difference \\
\midrule
Corridor length per hop (m) & 3.339 & 3.588 & +7.5\% (hex longer) \\
\textbf{Mean NS$\to$room distance (m)} & \textbf{8.68} & \textbf{6.94} & \textbf{-20.1\% (hex shorter)} \\
Max NS$\to$room distance (m) & 13.36 & 10.76 & -19.5\% (hex shorter) \\
\textbf{Optimal TSP tour length (m)} & \textbf{60.1} & \textbf{61.0} & +1.5\% (square shorter) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[noitemsep]
  \item \textbf{Individual trips are 20.1\% shorter in the hexagonal layout.}
    Despite each hop being 7.5\% physically longer ($l_h\sqrt{3} = 3.588\,\text{m}$
    vs $l_s = 3.339\,\text{m}$), the hexagonal layout's 6-connectivity provides
    diagonal shortcuts that reduce the number of hops required to reach any patient
    room, yielding a mean travel distance of \SI{6.94}{\metre} versus \SI{8.68}{\metre}
    in the square layout ($\Delta = 1.74\,\text{m}$, $-20.1\%$).
    The maximum distance to the most remote room also decreases from
    \SI{13.36}{\metre} to \SI{10.76}{\metre} ($-19.5\%$).

  \item \textbf{The optimal full-round tour is nearly identical.}
    The TSP optimum for a complete round of all 15 rooms is \SI{60.1}{\metre} for
    the square layout and \SI{61.0}{\metre} for the hexagonal layout
    (difference: $0.9\,\text{m}$, $<2\%$, in favour of square).
    The regular serpentine structure of the square grid allows the TSP solver to plan
    efficient sweeping routes; the hexagonal diagonal shortcuts do not further improve
    the global round because all rooms must be visited regardless.
\end{enumerate}

Figure~\ref{fig:tsp_tour} shows the optimal tour overlaid on each layout graph.
Figure~\ref{fig:tsp_heatmap} presents a colour-coded heatmap of individual room
distances from the nursing station.
Figure~\ref{fig:tsp_dist} compares the distance distributions and summary metrics.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/tsp_tour_comparison.pdf}
\caption{Optimal MTZ TSP nurse tour (red path) on each layout graph.
Green square = nursing-station depot.
Blue = nursing-station modules.
Red circles = patient rooms.
Grey edges = all available corridors.
Square optimal tour: \SI{60.1}{\metre}.
Hexagonal optimal tour: \SI{61.0}{\metre} ($<$2\% difference).}
\label{fig:tsp_tour}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/tsp_heatmap.pdf}
\caption{Heatmap of shortest-path distance from the nursing-station depot to each
patient room (green $=$ depot; warm red $=$ far rooms).
The hexagonal layout's 6-connectivity eliminates the deep-red ``far corners''
present in the square layout, reducing the maximum reach by 19.5\%.}
\label{fig:tsp_heatmap}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/tsp_distance_comparison.pdf}
\caption{Left: individual room distances from the nursing-station depot (sorted).
Right: summary comparison of mean distance and optimal tour length.
Hex layout is superior for individual trips (-20.1\%) but equivalent for the
full daily round ($<$2\% difference).}
\label{fig:tsp_dist}
\end{figure}
...
\subsection{The travel-distance reduction is the key quantitative finding}

The graph-model optimisation provides the paper's most policy-relevant result:
the hexagonal layout reduces mean nurse travel distance by 20.1\%
(8.68\,m to 6.94\,m per trip), with the maximum reach shrinking by 19.5\%.
This reduction arises because 6-connectivity allows diagonal shortcuts that
offset the 7.5\% longer per-hop distance.

Importantly, the optimal full-round tour (TSP) is nearly identical for both
layouts (60.1\,m vs 61.0\,m, $<$2\% difference).
This nuanced result has practical implications: \textbf{hexagonal layouts primarily
benefit urgent, single-room response} (e.g., a nurse responding to an alarm),
while scheduled full-round inspections see negligible distance savings.

Clinical significance: a nurse making 8--10 urgent room visits per shift saves
approximately 14--17\,m of walking per shift, compounding to
50--60\,m per day across a 12-hour shift---consistent with emerging evidence that
proximity to patients improves response time and reduces medication errors
\cite{friesen2008nursing,ulrich2008evidence}.

The crossing-point metric (4--6 vs.\ 2--3 per route) provides a complementary
structural interpretation of the same advantage: more branch options reduce the
probability of a blocked corridor forcing a circuitous route.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Systems not modelled.}
    ...
    analysed.
  \item \textbf{Graph model assumes full corridor connectivity.}
    The TSP graph assumes every pair of adjacent rooms has an open corridor.
    In reality, walls are selectively removed; the exact corridor network
    depends on the specific floor plan and differs from the full-adjacency model
    used here.
    The 20.1\% travel-distance finding should be interpreted as an upper bound on
    the advantage achievable with an optimally designed hexagonal corridor network.
  \item \textbf{No dynamic simulation.}
    The MTZ TSP finds the optimal static route; it does not model stochastic
    events (corridor blockages, simultaneous room calls, nurse fatigue).
    A discrete-event simulation is the recommended next step.
\end{enumerate}
...
\begin{itemize}[noitemsep]
  \item The theoretical 6.94\% perimeter saving is a geometric identity (honeycomb
    conjecture), constant across all module areas, but it reverses at the floor-plan
    level: the specific hexagonal design requires 12.0\% more wall material after
    wall-sharing and passage accounting.
  \item Graph-model optimisation (Dijkstra + exact MTZ TSP) shows that the hexagonal
    layout reduces mean nurse travel distance by \textbf{20.1\%}
    (\SI{8.68}{\metre} to \SI{6.94}{\metre}), with negligible difference in the
    optimal full-round tour ($<$2\%).
  \item The 20.1\% individual-trip reduction is the primary quantitative argument for
    hexagonal nursing stations; it directly translates to faster response to urgent calls
    and is independent of wall-material accounting.
  \item Expert perception (mockup) confirms that functional concern is low, but
    stakeholder unfamiliarity with non-rectangular forms is the principal adoption barrier.
\end{itemize}

The primary recommendation for practitioners: do not rely on the 6.94\% material
saving---run the adjusted wall-sharing analysis for your specific floor plan.
The primary clinical argument is the 20.1\% travel-distance reduction, which
should be validated with discrete-event simulation in a real Mexican hospital unit
and reviewed against NOM-016-SSA3-2012.
